...
#This class handles all the communications and protocol with the Tracker.
class Rerequester(object):
    ...
    def __init__(self, url, config, sched, howmany, connect, externalsched,
            ...
            upratefunc, downratefunc, ever_got_incoming, diefunc, sfunc, logcollector):
        self.baseurl = url
    ...
    #calling events: begin -> _check -> announce -> _request -> _postrequest
    def begin(self):
    ...
    #create the Thread to request the tracker
    def _announce(self, event=None):
        ...
        if self.howmany() >= self.config['max_initiate']:
            ...
            #with compact mode, peer client IDs are not sent.
            s += '&compact=1'
    ...
    #post process the data received from the tracker
    def _postrequest(self, data=None, errormsg=None, peerid=None):
        ...
        try:
            ...
            self._fail()
            return
        #check if the tracker rejected the peer request
        if r.has_key('failure reason'):
            if self.howmany() > 0:
                ...
                self.sched(die, 0)
            self._fail()
        #the tracker has accepted the request and has returned a valid message
        else:
            self.fail_wait = None
            ...
            self.logcollector.log(None, 'RT ID ' + str(self.trackerid) +' AI '+ str(self.announce_interval))
            ...
            self.logcollector.log(None, 'RT NP ' +  str(len(peers)))
            for x in peers:
                self.logcollector.log(None, 'RT P ' + str(x[0]) + ' ' + str(x[1]) + ' ' + str(x[2]))
                #call Encoder.start_connection(self, dns, id).
                #The tuple (x[0], x[1]) represents the dns and x[2] the peer ID
