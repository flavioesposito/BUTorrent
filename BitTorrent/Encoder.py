...
class Encoder(object):
    ...
    def __init__(self, make_upload, downloader, choker, numpieces, ratelimiter,
               raw_server, config, my_id, schedulefunc, download_id, context, logcollector):
    ...
    def start_connection(self, dns, id):
        ...
        if len(self.connections) >= self.config['max_initiate']:
            ...
            return
        self.logcollector.log(None,'CON ST ' + str(dns[0]))
        try:
            c = self.raw_server.start_connection(dns, None, self.context)
        except socketerror:
            self.logcollector.log(None,'CON SE ' + str(dns[0]))
            pass
        else:
            self.logcollector.log(None,'CON SS ' + str(dns[0]))
            con = Connection(self, c, id, True, self.logcollector)
            self.connections[c] = con
            c.handler = con

    #called when the connection handshake is done
    def connection_completed(self, c):
        if c.locally_initiated:
            self.logcollector.log(None,'CONH L ' + str(c.ip) + ' ID ' + str(c.id))
        else:
            self.logcollector.log(None,'CONH R ' + str(c.ip) + ' ID ' + str(c.id))
...
class SingleportListener(object):
    ...
    def __init__(self, rawserver, logcollector):
    ...
    def external_connection_made(self, connection):
        con = Connection(self, connection, None, False, self.logcollector)
