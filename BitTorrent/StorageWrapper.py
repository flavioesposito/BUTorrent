...
class StorageWrapper(object):
    ...
    def __init__(self, storage, config, hashes, piece_size, finished,
            statusfunc, flag, data_flunked, infohash, errorfunc, resumefile, logcollector):
    ...
    #return true if there are still blocks missing for the piece with ID index
    #or if the piece was not downloaded at all yet.
    ...
    def new_request(self, index):
        ...
        #amount_inactive is the amount of data that still need to be requested to
        #peer. When amount_inactive==0 there is no more data to be requested. All
        #the data are either already received or already requested. 
        if self.amount_inactive == 0:
            self.endgame = True
            self.logcollector.log('End game mode', 'P EG')
        return r
