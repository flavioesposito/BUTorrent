...
class Multitorrent(object):
    ...
    def start_torrent(self, metainfo, config, feedback, filename):
        torrent = _SingleTorrent(self.rawserver, self.singleport_listener,
                                 self.ratelimiter, self.filepool, config, self.logcollector)
        self.rawserver.add_context(torrent)

        #function called by the scheduler in RawServer when it the time for this torrent to start
        def start():
            torrent.start_download(metainfo, feedback, filename)

        #initial schedule of the torrent
...
class _SingleTorrent(object):
    ...
    def _finished(self):
        self.logcollector.log(None, 'P SM')
        self.finflag.set()
        ...
        if self._announced:
            ...
            # program terminate options when leecher becomes seed
            if self.config['shutdown_after_seed']:
                self._rawserver.doneflag.set() # PFS program terminate
    ...
    #with default values, max_uploads_internal is set to 4
    def _set_auto_uploads(self):
        #max_uploads set to -1 by default
        uploads = self.config['max_uploads']
        #max_upload_rate: maximum kB/s to upload at
